  ...
  def self.query(prompt, model)
    ...
    begin
      ...
      if response.code == 200
        ...
        [image_response.body, response.code]
      elsif response.code == 429
        puts 'DALL-E3のレート制限に達しました。DALL-E2モデルへフォールバックします。'
        %w[x-ratelimit-limit-requests x-ratelimit-remaining-requests x-ratelimit-reset-requests
           x-ratelimit-limit-tokens x-ratelimit-remaining-tokens x-ratelimit-reset-tokens].each do |header|
